...
df_trajectory_summaries_noage <- df_trajectory_full  %>% group_by(time, exposure_type, biomarker) %>% mean_qi(value)
...
df_trajectory_o5 <- df_trajectory_full %>% group_by(exposure_type, biomarker, covar, sample) %>% filter(exposure_type != "none") %>% 
    mutate(above_5 = value >= log10(4)) %>% filter(above_5) %>% group_by(exposure_type, biomarker, covar, sample) %>% filter(time == max(time)) %>%
    mutate(age_group = recode(covar, `1` = "≤5", `2` = "5-18", `3` = "19-59", `4` = "60-74", `5` = "75+")) %>% 
    mutate(age_group = factor(age_group, levels = c("≤5", "5-18", "19-59", "60-74", "75+"))) 
...
df_trajectory_compare %>% mutate(above_5 = value >= 5)
...
p2 <- df_trajectory_o5 %>% 
    ggplot() + stat_pointinterval(aes(x = age_group, y = time, color = age_group)) + theme_bw() + 
    labs(x = "Age group (years)", y = "Number of days above a four-fold rise") + theme_ft() + theme(legend.position = "none") 
...
sum_func <- function(time) {
  tibble(
    mean = mean(time),
    lb =  quantile(time, c(0.025)),
    ub =  quantile(time, c(0.975))
  ) 
}
...
df_trajectory_max <- df_trajectory_full %>% group_by(exposure_type, biomarker, covar, sample) %>% filter(value == max(value)) %>% filter(exposure_type != "none")
df_trajectory_max %>% group_by(exposure_type, biomarker, covar) %>% summarise(10^sum_func(value))

# persistence 
df_trajectory_o5 %>% group_by(exposure_type, biomarker, covar) %>% summarise(sum_func(time))


df_o4_all <- df_trajectory_full %>% filter(exposure_type == "inf") %>%
