...
data_titre <- read.csv( file = here::here("data_clean", "fudan", "df_soro_model.csv")) %>% select(!X) 
...
abkineticsModel <- list(
    model = makeModel(
            ...
            addAbkineticsModel("inf", "PreF", "inf", c("y1", "t1", "r"), infTuenisPower2016)
        ),
    prior = bind_rows(
        addPrior("y1", 0.5, 6, "unif",  0.5, 6), # ab kinetics
        addPrior("t1", 7, 24, "norm", 14, 3), # ab kinetics
        addPrior("r", 1, 5, "unif", 1, 5), # ab kinetics 
...
save_info_list <- list(
    list(file_name = "fudan_e3", model_name = "base"),
    list(file_name = "fudan_e3", model_name = "inform"),
    list(file_name = "fudan_e3_hpc", model_name = "base"),
    list(file_name = "fudan_e3_hpc", model_name = "inform")
)
