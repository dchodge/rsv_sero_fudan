...
abkineticsModel <- list(
    model = makeModel(
            ...
            addAbkineticsModelHier("inf", "PreF", "inf", c("y1", "t1", "r"), c("y1", "t1", "r"), hierdata, infTuenisPower2016)
        ),
    prior = bind_rows(
        addPriorHier("y1", -10, 10, "norm",  0, 1.82, "lnorm", -1, 1, M, c(0.5, 6) ), # ab kinetics
        addPriorHier("t1", -10, 10, "norm", 0, 1.82, "lnorm", -1, 1, M, c(7, 24) ), # ab kinetics
...
save_info_list <- list(
    list(file_name = "fudan_e3", model_name = "base_hier_3"),
    list(file_name = "fudan_e3", model_name = "inform_hier_3"),
    list(file_name = "fudan_e3_hpc", model_name = "base_hier_3"),
    list(file_name = "fudan_e3_hpc", model_name = "inform_hier_3")
)

model_run_info <- 
    list(model = outputs_modelA, save = save_info_list)
saveRDS(model_run_info, file = here::here("models", "model_base_hier_3.RDS"))
